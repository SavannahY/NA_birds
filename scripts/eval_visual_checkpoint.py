#!/usr/bin/env python3
"""Evaluate a trained NABirds visual checkpoint on a manifest split."""
...
from scripts.train_visual_baseline import (  # noqa: E402
    _limit_rows,
    _read_manifest,
...
def _load_class_names(path: Optional[Path]) -> Dict[int, str]:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            if "target" not in row or "class_name" not in row:
                continue
...
def _resolve_from_checkpoint(args: argparse.Namespace, checkpoint: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "input_mode": args.input_mode or ckpt_args.get("input_mode", "full"),
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    dataset = Dataset(
        rows,
        build_transforms(transforms, resolved["image_size"], train=False),
        resolved["input_mode"],
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained NABirds visual checkpoint.")
    ...
    parser.add_argument("--input-mode", choices=["", "full", "bbox"], default="", help="Override input mode.")
...
def main() -> None:
    ...
    print("NABirds visual checkpoint evaluation")
